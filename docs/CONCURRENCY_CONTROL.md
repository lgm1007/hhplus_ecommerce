# 동시성 제어 방식 분석
동시성 제어를 하기 위해 알아보고 적용해 본 방식들을 정리한 문서이다.
...
이유: 같은 상품에 대해 주문이 몰리는 시나리오는 유명한 제품일 경우 자주 발생할 수 있는 시나리오라고 판단했다.
...
분산락 방식 중 심플락은 락 획득 시 예외 처리하기 때문에 해당 시나리오에서 부적합하고,
스핀락은 재시도로 인해 지속적인 재시도 실패 시 스레드 점유에 대한 문제가 있다.
따라서 분산락 중 Pub/Sub 방식으로 해결하고자 한다.
```
