...
import com.example.hhplus_ecommerce.payment.request.PaymentRequest
import com.example.hhplus_ecommerce.payment.response.PaymentResponse
	...
	fun payment(@RequestBody paymentRequest: PaymentRequest): ResponseEntity<PaymentResponse> {
		return ResponseEntity.ok(
			PaymentResponse.from(
				paymentResultInfoDto = paymentFacade.orderPayment(userId = paymentRequest.userId, orderId = paymentRequest.orderId)
			)
		)
	}
}