...
import com.example.hhplus_ecommerce.order.request.OrderRequest
import com.example.hhplus_ecommerce.order.response.OrderResponse
...
class OrderApi(private val orderFacade: OrderFacade) {
	...
	fun order(@RequestBody orderRequest: OrderRequest): ResponseEntity<OrderResponse> {
		return ResponseEntity.ok(
			OrderResponse.from(
				orderInfo = orderFacade.productOrder(userId = orderRequest.userId, orderItemInfoDtos = orderRequest.orderItemInfos)
			)
		)
	}
}