...
class BalanceServiceIntegrationTest {
	...
	fun getBalanceByUserId() {
		...
		val actual = balanceService.getByUserId(userId = 298765L)
	...
	fun balanceDecreaseOnce() {
		val balance = BalanceEntity(userId = 1L, amount = 10000)
		balanceRepository.save(balance)

		balanceService.updateAmountDecrease(userId = 1L, amount = 8000)
		val actual = balanceService.getByUserId(userId = 1L)
	...
	fun shouldFailAmountNotEnough() {
		val balance = BalanceEntity(userId = 1L, amount = 5000)
		balanceRepository.save(balance)

		assertThatThrownBy { balanceService.updateAmountDecrease(userId = 1L, amount = 10000) }
	...
	fun balanceChargeOnce() {
		...
		balanceService.updateAmountCharge(userId = 1L, amount = 10000)
		val actual = balanceService.getByUserId(userId = 1L)
	...
	fun shouldFailChargeAmountNegative() {
		...
		assertThatThrownBy { balanceService.updateAmountCharge(userId = 1L, amount = -10000) }
			.isInstanceOf(BadRequestException::class.java)
			.extracting("errorStatus")
	...
	fun balanceChargeConcurrency() {
		...
		// 예상 충전 잔액: 10,000
		val balance = BalanceEntity(userId = 1L, amount = 0)
		balanceRepository.save(balance)
