...
import com.example.hhplus_ecommerce.lock.RedisLockSupporter
import com.example.hhplus_ecommerce.product.dto.ProductDetailDto
import com.example.hhplus_ecommerce.product.dto.ProductDto
...
class ProductService(
	...
	fun getAllProductInfosWithPaging(page: Int, itemSize: Int): List<ProductInfoDto> {
		val products = productRepository.getAllByPaging(page = page, itemSize = itemSize).map(ProductDto::from)
		val productIds = products.map { it.productId!! }
		val productDetails = productDetailRepository.getAllByProductIdsIn(productIds).map(ProductDetailDto::from)

		val productInfos = ProductInfoDto.listOf(products, productDetails)
	...
	@Transactional(readOnly = true)
	fun getProductInfoById(productId: Long): ProductInfoDto {
		...
		return ProductInfoDto.of(product, productDetail)
	}
	...
	@Transactional(readOnly = true)
	fun getAllProductStatisticsInfos(productDetailIds: List<Long>): List<ProductStatisticsInfoDto> {
		...
		return ProductStatisticsInfoDto.listOf(productDtos, productDetailDtos)
	}
